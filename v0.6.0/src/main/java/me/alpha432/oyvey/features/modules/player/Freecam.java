...
public class Freecam
    ...
    public void onUpdate() {
        ...
        if (Freecam.mc.gameSettings.keyBindJump.isKeyDown()) {
            Freecam.mc.player.motionY += this.speed.getValue();
        }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        Entity riding;
        if (event.getPacket() instanceof SPacketSetPassengers && (riding = Freecam.mc.world.getEntityByID((packet = event.getPacket()).getEntityId())) != null && riding == this.riding) {
            this.riding = null;
        }
        if (event.getPacket() instanceof SPacketPlayerPosLook) {
            SPacketPlayerPosLook packet1 = event.getPacket();
            if (this.packet.getValue()) {
                if (this.entity != null) {
                ...
                Freecam.mc.player.connection.sendPacket(new CPacketConfirmTeleport(packet1.getTeleportId()));
                event.setCanceled(true);
            } else {
                event.setCanceled(true);
            }
